...
export function saveUsageStats({ provider, model, tokens, connectionId, apiKey, endpoint, latency, status, label = "USAGE", silent = false }) {
  ...
  saveRequestUsage({
    ...
    latencyMs: typeof latency === "number" && latency > 0 ? Math.round(latency) : 0,
    status: status || "ok",
  }).catch(() => {});
}
