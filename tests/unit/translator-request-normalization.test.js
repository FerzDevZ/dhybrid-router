...
describe("request normalization", () => {
  it("claudeToOpenAIRequest preserves text content blocks in arrays", () => {
    const body = {
      messages: [
        {
          role: "user",
  ...
  it("translateRequest keeps /v1/messages Claude->OpenAI text payloads as content blocks", () => {
    const body = {
  ...
  it("parseSSELine supports provider raw NDJSON stream lines (ollama format only)", () => {
    ...
    // NDJSON requires the explicit format arg — otherwise it's not an SSE line
