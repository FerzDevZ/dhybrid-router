// Per-API-key request throttling:
 ...
 import { maskKey } from "./maskKey.js";
 ...
 export function checkKeyRateLimit(key, { enabled, rpm = 60, windowMs = 60_000 } = {}) {
   ...
   // Opportunistic prune: drop expired entries to prevent unbounded growth (Fix 16)
...
export function resetRateLimiter() {
 ...
 export async function checkDailyQuota(key, { enabled, requests = 500 } = {}) {
   ...
   // Atomic increment: use better-sqlite3's RETURNING via kvStore's underlying DB
   // Fallback to read-then-write if not available.
   try {
     const db = quotaKv.getDb?.();
     if (db?.prepare) {
       // Direct SQL with atomic increment and RETURNING
       const stmt = db.prepare(`
         INSERT INTO kv (key, value, expiresAt)
         VALUES (?, 1, strftime('%s', date('now', '+1 day')))
         ON CONFLICT(key) DO UPDATE SET value = value + 1
         RETURNING value
       `);
       const row = stmt.get(k);
       const used = row?.value ?? 1;
       if (used > requests) return { allowed: false, used, limit: requests };
       return { allowed: true, used, limit: requests };
     }
   } catch {
     // Fall through to non-atomic path
   }
   // Non-atomic fallback (race window but works)
 ...
 export function rateLimitResponse(reason, retryAfterSec, status = HTTP_STATUS.RATE_LIMITED) {
